"""Hazır agent sağlayıcıları: provider seçimi + başlatıcıların doğru
...
def test_build_agent_command_known_providers():
    for provider, launcher in [("opencode", "opencode.py"),
...
def test_build_agent_command_unknown_provider():
...
def test_opencode_launcher_passes_prompt_and_model(tmp_path):
    args = _run_launcher(tmp_path, "opencode.py",
                         {"HEAL_MODEL": "vllm/glm-5.2-fp8"})
    assert args[0] == "run"
    assert args[1:3] == ["--model", "vllm/glm-5.2-fp8"]
    assert "Kampanya sepeti" in args[-1]  # prompt içerik olarak geçildi


def test_claude_code_launcher_flags(tmp_path):
    args = _run_launcher(tmp_path, "claude_code.py",
                         {"HEAL_MODEL": "glm-5.2-fp8"})
...
def test_aider_launcher_uses_message_file(tmp_path):
    args = _run_launcher(tmp_path, "aider.py",
                         {"HEAL_MODEL": "openai/glm-5.2-fp8"})
    assert "--yes-always" in args
    assert "--no-auto-commits" in args  # commit'i healing motoru atar
    file_index = args.index("--message-file")
    assert args[file_index + 1].endswith("HEAL_TASK.md")
